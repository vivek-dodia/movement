'use client'

import Link from 'next/link'
import { useEffect, useState } from 'react'
import { motion } from 'framer-motion'
import { usePathname } from 'next/navigation'
import { useNavContext } from '../context/NavContext'

const navigationOptions = [
  { name: 'Dashboard', href: '/dashboard', active: false },
  { name: 'Workouts', href: '/workouts', active: false },
  { name: 'Diary', href: '/diary', active: false },
  { name: 'Locations', href: '/locations', active: false },
] as const

type SectionNames = (typeof navigationOptions)[number]['name'] // 'Dashboard' | 'Workouts' | 'Diary' | 'Locations'

export default function NavbarLinks() {
  const [activeSection, setActiveSection] = useState<SectionNames | null>(null)
  const { setIsNavExpanded } = useNavContext()
  const pathname = usePathname()

  useEffect(() => {
    if (pathname === '/dashboard') {
      setActiveSection('Dashboard')
    } else if (pathname === '/workouts') {
      setActiveSection('Workouts')
    } else if (pathname === '/diary') {
      setActiveSection('Diary')
    } else if (pathname === '/locations') {
      setActiveSection('Locations')
    } else {
      setActiveSection(null)
    }
  }, [])

  return (
    <ul className=" gap-2 flex flex-col border-b border-b-gray-50/10 pb-5  lg:flex-row lg:border-none lg:py-0">
      {navigationOptions.map((option, i) => {
        return (
          <Link
            href={option.href}
            key={option.href}
            onClick={() => {
              setActiveSection(option.name)
              setIsNavExpanded(false)
            }}
            className="relative"
          >
            <li
              className={`text-gray-200 font-medium text-[.95rem] px-5 py-2 transition-all rounded-lg cursor-pointer z-[1] relative mx-2 lg:mx-0  ${
                activeSection === option.name
                  ? ' text-gray-50'
                  : 'hover:bg-blue-500 hover:text-gray-100'
              }`}
            >
              {option.name}
              {option.name === activeSection && (
                <motion.span
                  layoutId="activeSection"
                  transition={{
                    type: 'spring',
                    stiffness: 300,
                    damping: 25,
                  }}
                  className="bg-blue-700 rounded-lg absolute inset-0 -z-10"
                ></motion.span>
              )}
            </li>
          </Link>
        )
      })}
    </ul>
  )
}
